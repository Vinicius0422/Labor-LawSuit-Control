...
@RestController
@RequestMapping(value = "api/v1/defendants", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
...
public class DefendantController {
    ...
    public ResponseEntity findAllDefendants(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size){
        try{
            Page<DefendantSomeFieldsResponseDto> defendantResponse = defendantService.getAllDefendants(page, size);
            return ResponseEntity.ok().body(defendantResponse);
        } catch(Exception e){
    ...
    public ResponseEntity findDefendantById(@Parameter(description = "ID of the defendant", required = true) @PathVariable(value = "defendantId") Long defendantId){
        try{
            DefendantResponseDto defendantResponse = defendantService.getDefendantById(defendantId);
            return ResponseEntity.ok().body(defendantResponse);
        } catch(Exception e){
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage());
        }
    ...
    public ResponseEntity findDefendantByCpfOrCnpj(@Parameter(description = "CPF/CNPJ of the defendant", required = true) @RequestParam String cpfCnpj){
        try{
            DefendantResponseDto defendantResponse = defendantService.getDefendantByCpfOrCnpj(cpfCnpj);
            return ResponseEntity.ok().body(defendantResponse);
        } catch(Exception e){
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage());
        }
    ...
    public ResponseEntity findDefendantsByName(@Parameter(description = "Name of the defendant", required = true) @RequestParam String defendantName,
        ...
        try{
            Page<DefendantResponseDto> defendantResponse = defendantService.getDefendantByName(defendantName, page, size);
            return ResponseEntity.ok().body(defendantResponse);
        } catch(Exception e){
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage());
        }
    ...
    public ResponseEntity saveDefendant(@io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Request body containing defendant data", required = true,
            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE)) @RequestBody @Valid DefendantRequestDto defendantRequestDto){
        try{
            var defendantResponse = defendantService.saveDefendant(defendantRequestDto);
            return ResponseEntity.status(defendantResponse.statusCode()).body(defendantResponse.message());
        } catch(Exception e){
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage());
        }
    }
